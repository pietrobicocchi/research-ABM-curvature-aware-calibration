...
\end{equation}
Near a minimum of the unbiased MMD U-statistic
\citep{gretton_mmd_2012}, the loss takes a residual form in which the
$g_m$ behave as per-seed Jacobian rows of a vanishing residual: the
per-seed outer product $\widehat F$ is then a stochastic generalised
Gauss--Newton (GGN) approximation of the loss Hessian, independent of
any likelihood interpretation. We use this reading throughout as
motivation for the eigendecomposition, not as a uniform-convergence
claim. The consequence set out in \S\ref{sec:intro} follows: read this
