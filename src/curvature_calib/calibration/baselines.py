...
def sgd(simulate_fn: Callable, theta0, Y_ref, M=64, n_iter=80,
        lr=1e-2, seed_base=0, verbose=False) -> FirstOrderLog:
    theta = theta0
    log = FirstOrderLog(thetas=[theta], losses=[], mean_grads=[])
    ...
    for t in range(n_iter):
        ...
        if verbose and t % 10 == 0:
            print(f"  SGD iter {t}  L={float(stats.loss):.4e}")
...
def adam(simulate_fn: Callable, theta0, Y_ref, M=64, n_iter=80,
    ...
    for t in range(1, n_iter + 1):
        ...
        log.thetas.append(theta)
        if verbose and t % 10 == 0:
            print(f"  Adam iter {t}  L={float(stats.loss):.4e}")
