"""Figure 6 - Falsification (mean-field SIR, interventionist lockdown).

Companion to fig4 for the SIR model. theta* uses an interventionist lockdown
(fires day 16, before the epidemic peak ~day 29) so the lockdown actually bends
the curve and the sloppy direction is a genuine beta-vs-lockdown trade-off
combination rather than an inert coordinate. 2x3 grid: rows = stiff v_1 / sloppy
v_P, columns = moments / ACF sup-norm / tail quantiles, discrepancy vs signed
step alpha. Shared y per column. Unlike Brock-Hommes the SIR sloppy direction is
suppressed but not invisible (~10x), which the figure reports honestly.
"""
...
T = 200
THETA_STAR = jnp.array([0.40, 0.10, 1e-3, 0.08, 0.50], dtype=jnp.float64)  # lockday 16
M = 96
N_BATCH = 6
ALPHAS = np.linspace(-0.06, 0.06, 13)
...
theta_eval = THETA_STAR + jnp.array([0.01, -0.005, 1e-4, 0.005, -0.02], dtype=jnp.float64)
...
for ci, ch in enumerate(CHANNELS):
    ...
    for dname in DIRS:
        ...
        ax = axes[r, ci]
        arr = curves[dname][ci]
        ax.fill_between(ALPHAS, arr.min(0), arr.max(0), color=row_color[dname],
                        alpha=0.15, lw=0)
...
ps.save(fig, "fig6_sir_falsification")
print("saved fig6_sir_falsification")
